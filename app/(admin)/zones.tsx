...
async function fetchZones(): Promise<Zone[]> {
  if (typeof window !== 'undefined') {
    const res = await fetch(`${API_BASE}/api/admin?r=zones`);
...
async function toggleZoneActive(p: { id: string; isActive: boolean }) {
  if (typeof window !== 'undefined') {
    await fetch(`${API_BASE}/api/admin?r=zones`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(p) });
    return;
  }
  const { getDb } = await import('@/lib/db/client');
  const { fishingZones } = await import('@/lib/db/schema');
...
async function createZone(data: {
  ...
  if (typeof window !== 'undefined') {
    const res = await fetch(`${API_BASE}/api/admin?r=zones-create`, {
